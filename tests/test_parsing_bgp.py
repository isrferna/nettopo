...
`parsing/bgp.py`), the iBGP/eBGP classification the summary never states outright, and
the State/PfxRcd column in each of its two meanings: a prefix count for a session that is
up, and a state word for one that is not.
"""
...
def test_returns_nothing_when_the_command_was_not_captured() -> None:
    assert parse_bgp("r1", "r1#show version\nCisco IOS Software\n") == []


def test_parses_one_peer_per_neighbor_row() -> None:
    peers = parse_bgp("r1", _capture("ios_show_ip_bgp_summary.txt"))
...
def test_a_peer_carries_both_as_numbers_and_the_reporting_device() -> None:
    (ibgp, *_rest) = parse_bgp("r1", _capture("ios_show_ip_bgp_summary.txt"))
...
def test_iosxe_variant_parses_the_same_way() -> None:
    peers = parse_bgp("r1", _capture("iosxe_show_ip_bgp_summary.txt"))
...
def test_as_numbers_written_in_asdot_notation_are_converted() -> None:
    ...
    (peer,) = parse_bgp("r1", _ASDOT_CAPTURE)
...
def test_a_platform_without_a_template_warns_instead_of_raising(
    ...
    with caplog.at_level(logging.WARNING, logger="nettopo"):
        peers = parse_bgp("r1", _capture("ios_show_ip_bgp_summary.txt"), platform="juniper_junos")
