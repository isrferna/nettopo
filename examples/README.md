...
| `acc-sw3` | no | Access switch seen only in `dist-sw2`'s CDP output — drawn dashed in the STP view |
| `edge-rtr` | no | WAN router on a routed port, so it appears in the L2 view and not in the STP view |
| `esxi-host01`, `esxi-host02` | no | Hosts on Edge/PortFast ports: dropped by `--endpoints network-only` and by the STP view |
